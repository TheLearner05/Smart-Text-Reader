...
def text_detector(image):
    ...
    for (startX, startY, endX, endY) in boxes:
        ...
        orig = cv2.putText(orig, textRecongized, (endX, endY+5),
...
# op = text_detector(image0)
# cv2.imshow("Text Detection",op)
# cv2.waitKey(0)
...
while(True):
    ...
    if ret == True:
        img = frame
        imageO = cv2.resize(img, (512, 512), interpolation=cv2.INTER_AREA)
